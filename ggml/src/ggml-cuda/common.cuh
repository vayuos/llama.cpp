...
template <class... Archs> constexpr bool ggml_cuda_has_arch_impl(const int arch, const int first, Archs... rest) {
...
#ifndef NDEBUG
#    define GGML_ASSERT_CUDA(x) GGML_ASSERT(x)
#else
#    define GGML_ASSERT_CUDA(x)
#endif
...
[[noreturn]] static __device__ void no_device_code(const char * file_name,
                                                   const int    line,
...
#else
    printf(
        "%s:%d: ERROR: CUDA kernel %s has no device code compatible with CUDA arch %d. ggml-cuda.cu was compiled for: "
        "%s\n",
...
template <int n> struct ggml_cuda_unroll {
    template <typename Func, typename... Args> __device__ void operator()(const Func & f, Args... args) const {
        f(n - 1, args...);
        ggml_cuda_unroll<n - 1>{}(f, args...);
    }
};

template <> struct ggml_cuda_unroll<1> {
    template <typename Func, typename... Args> __device__ void operator()(const Func & f, Args... args) const {
        f(0, args...);
    }
};

template <int width = WARP_SIZE> static __device__ __forceinline__ int warp_reduce_sum(int x) {
#if !defined(GGML_USE_HIP) && __CUDA_ARCH__ >= GGML_CUDA_CC_AMPERE
...
template <int width = WARP_SIZE> static __device__ __forceinline__ float2 warp_reduce_sum(float2 a) {
#pragma unroll
    for (int offset = width / 2; offset > 0; offset >>= 1) {
...
template <int width = WARP_SIZE> static __device__ __forceinline__ half2 warp_reduce_sum(half2 a) {
...
template <int width = WARP_SIZE> static __device__ __forceinline__ int warp_reduce_all(int x) {
...
template <int width = WARP_SIZE> static __device__ __forceinline__ int warp_reduce_any(int x) {
    if (width == ggml_cuda_get_physical_warp_size()) {
...
template <typename T, int width = WARP_SIZE> static __device__ __forceinline__ T warp_prefix_inclusive_sum(T x) {
...
template <int width = WARP_SIZE> static __device__ __forceinline__ float2 warp_prefix_inclusive_sum(float2 a) {
    const int lane_id = threadIdx.x % width;
#pragma unroll
    for (int offset = 1; offset < width; offset <<= 1) {
...
template <int width = WARP_SIZE> static __device__ __forceinline__ half2 warp_prefix_inclusive_sum(half2 a) {
#ifdef FP16_AVAILABLE
    const int lane_id = threadIdx.x % width;
#    pragma unroll
    for (int offset = 1; offset < width; offset <<= 1) {
...
template <block_reduce_method method_t, typename T> struct block_reduce_policy;

template <typename T, typename... Ts> inline constexpr bool is_any = (std::is_same_v<T, Ts> || ...);

template <typename...> inline constexpr bool ggml_cuda_dependent_false_v = false;
...
template <int width = WARP_SIZE> static __device__ __forceinline__ half2 warp_reduce_max(half2 x) {
...
static __device__ __forceinline__ void ggml_cuda_memcpy_1(void * __restrict__ dst, const void * __restrict__ src) {
    static_assert(nbytes <= ggml_cuda_get_max_cpy_bytes() || alignment == 0,
                  ...
                  "The intent is for the parameter is only as a workaround if either one of the pointers is not "
                  "properly aligned. "
                  "If you use it to do more bytes per copy than ggml_cuda_max_cpy_bytes() the reads and writes may not "
                  "be coalesced. "
...
template <ggml_type type> struct ggml_cuda_type_traits;
...
static_assert(std::is_trivial<ggml_cuda_graph_node_properties>::value,
              "ggml_cuda_graph_node_properties must be trivial");

struct ggml_cuda_graph {
#ifdef USE_CUDA_GRAPH
    ~ggml_cuda_graph() {
        // Prevent destroying persistent decode graphs while in decode mode.
        if (ggml_get_decode_mode()) {
            GGML_LOG_ERROR("%s: attempt to destroy cuda graph while decode mode active - aborting\n", __func__);
            GGML_ABORT("attempt to destroy cuda graph while decode mode active");
        }
        if (instance != nullptr) {
...
// Decode-mode globals and APIs (defined in ggml-cuda.cu)
extern const void *      ggml_cuda_decode_graph_key;
extern int              ggml_cuda_decode_device;
extern std::mutex       ggml_cuda_decode_mutex;
extern uint64_t         ggml_cuda_decode_sync_counter;
extern uint64_t         ggml_cuda_decode_poll_counter;
extern cudaStream_t     ggml_cuda_decode_stream;

extern void ggml_cuda_enter_decode_mode(const void * graph_key, struct ggml_backend_cuda_context * cuda_ctx);
extern void ggml_cuda_exit_decode_mode();
extern void ggml_cuda_reset_decode_sync_counter();
extern void ggml_cuda_reset_decode_poll_counter();
extern void ggml_cuda_assert_decode_invariant(const void * graph_key, struct ggml_backend_cuda_context * cuda_ctx);
extern void ggml_cuda_reset_decode_launch_counter();
extern uint64_t ggml_cuda_get_decode_launch_counter();

// ============================================================================
// Decode-path synchronization guards
// ============================================================================
// cudaDeviceSynchronize() and cudaStreamSynchronize() are strictly regulated
// during decode. Only ONE host-visible sync is permitted per token step.
//
// Usage: GGML_CUDA_SYNC_CHECK() must be called before any host-blocking
// sync operation in the decode path.

#define GGML_CUDA_ASSERT_NO_POLLING() \
    ([]() { \
        if (ggml_get_decode_mode()) { \
            GGML_LOG_ERROR("%s: FATAL: prohibited polling of GPU status during decode\n", __func__); \
            GGML_ABORT("prohibited polling during decode"); \
        } \
    }())

// Trap prohibited polling calls
#define cudaStreamQuery(s) (GGML_CUDA_ASSERT_NO_POLLING(), cudaStreamQuery(s))
#define cudaEventQuery(e)  (GGML_CUDA_ASSERT_NO_POLLING(), cudaEventQuery(e))
#define hipStreamQuery(s)  (GGML_CUDA_ASSERT_NO_POLLING(), hipStreamQuery(s))
#define hipEventQuery(e)   (GGML_CUDA_ASSERT_NO_POLLING(), hipEventQuery(e))

#define GGML_CUDA_ASSERT_SAME_STREAM(s) \
    do { \
        if (ggml_get_decode_mode() && ggml_cuda_decode_stream != nullptr) { \
            if ((cudaStream_t)(s) != ggml_cuda_decode_stream) { \
                GGML_LOG_ERROR("%s: FATAL: stream divergence during decode (expected=%p actual=%p)\n", \
                               __func__, (void*)ggml_cuda_decode_stream, (void*)(s)); \
                GGML_ABORT("stream divergence during decode"); \
            } \
        } \
    } while(0)

#define GGML_CUDA_SYNC_CHECK() \
    do { \
        if (ggml_get_decode_mode()) { \
...
#define GGML_CUDA_ASSERT_NO_DEVICE_SYNC() \
    do { \
        if (ggml_get_decode_mode()) { \
            GGML_CUDA_SYNC_CHECK(); \
            GGML_LOG_ERROR("%s: WARNING: cudaDeviceSynchronize attempted during decode at %s:%d\n", \
...
#define GGML_CUDA_ASSERT_NO_DEVICE_SYNC() \
    do { \
        if (ggml_get_decode_mode()) { \
            GGML_CUDA_SYNC_CHECK(); \
            GGML_LOG_ERROR("%s: FATAL: cudaDeviceSynchronize during decode at %s:%d\n", \
                           __func__, __FILE__, __LINE__); \
            GGML_ABORT("cudaDeviceSynchronize called during decode"); \
        } \
    } while(0)
#endif

// Guard for cudaStreamSynchronize during decode - increments counter and may abort.
#define GGML_CUDA_WARN_STREAM_SYNC_DECODE() \
    do { \
        if (ggml_get_decode_mode()) { \
            GGML_CUDA_SYNC_CHECK(); \
            GGML_LOG_DEBUG("%s: decode-path cudaStreamSynchronize at %s:%d (count=%lu)\n", \
                           __func__, __FILE__, __LINE__, (unsigned long)ggml_cuda_decode_sync_counter); \
        } \
    } while(0)
...
struct ggml_backend_cuda_context {
    ...
    explicit ggml_backend_cuda_context(int device) : device(device), name(GGML_CUDA_NAME + std::to_string(device)) {}
    ...
    cudaStream_t stream(int device, int stream) {
        ...
        cudaStream_t s = streams[device][stream];
...
struct ggml_cuda_mm_fusion_args_host {
    ...
    const ggml_tensor * rms_w     = nullptr;
    float               rms_eps;
    ggml_glu_op         glu_op;
