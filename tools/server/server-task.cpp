    ...
    server_sent_events.push_back(json{
        ...
        { "data",  json{
                      ...
                      { "response", json{ { "id", oai_resp_id },
                                          ...
                                          { "model", oaicompat_model },
                                          { "output", output },
                                          { "usage", json{ { "input_tokens", n_prompt_tokens },
                                                           { "output_tokens", n_decoded },
                                                           { "total_tokens", n_decoded + n_prompt_tokens } } } } },
                  }    }
    });
...
std::string server_task_result_cmpl_partial::to_json_string() {
    if (res_type == TASK_RESPONSE_TYPE_OAI_CHAT && oaicompat_msg_diffs.size() == 1) {
        const auto & diff = oaicompat_msg_diffs[0];
        if (diff.tool_call_index == std::string::npos && diff.reasoning_content_delta.empty() &&
            !diff.content_delta.empty() && !is_progress && prob_output.probs.empty() && timings.prompt_n < 0 &&
            !thinking_block_started && !text_block_started) {
            std::string       escaped = json(diff.content_delta).dump();
            std::stringstream ss;
            ss << R"([{"choices":[{"delta":{"content":)" << escaped
               << R"(},"finish_reason":null,"index":0}],"created":)" << std::time(nullptr) << R"(,"id":")"
               << oaicompat_cmpl_id << R"(","model":")" << oaicompat_model
               << R"(","object":"chat.completion.chunk","system_fingerprint":")"
               << "llamacpp" /* build_info not accessible easily, using placeholder */ << R"("}])";
