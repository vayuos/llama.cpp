...
struct server_metrics {
    ...
    std::atomic<uint64_t> n_prompt_tokens_processed_total{ 0 };
    std::atomic<uint64_t> t_prompt_processing_total{ 0 };
    std::atomic<uint64_t> n_tokens_predicted_total{ 0 };
    std::atomic<uint64_t> t_tokens_generation_total{ 0 };

    std::atomic<uint64_t> n_tokens_max{ 0 };

    std::atomic<uint64_t> n_prompt_tokens_processed{ 0 };
    std::atomic<uint64_t> t_prompt_processing{ 0 };

    std::atomic<uint64_t> n_tokens_predicted{ 0 };
    std::atomic<uint64_t> t_tokens_generation{ 0 };

    std::atomic<uint64_t> n_decode_total{ 0 };
    std::atomic<uint64_t> n_busy_slots_total{ 0 };
    ...
    void on_prompt_eval(const server_slot & slot) {
        ...
        uint64_t current_max = n_tokens_max.load();
        uint64_t new_tokens  = slot.prompt.n_tokens();
        while (current_max < new_tokens && !n_tokens_max.compare_exchange_weak(current_max, new_tokens))
            ;
    }
  ...
  private:
    ...
    bool init() {
        ...
        // populate chat template params
        {
            ...
            const bool enable_thinking = params_base.use_jinja && params_base.reasoning_budget != 0 &&
    ...
    void update_slots() {
                ...
                if (slot.state == SLOT_STATE_PROCESSING_PROMPT || slot.state == SLOT_STATE_STARTED) {
                        ...
                        if (!slot.can_split()) {
                            ...
                            if (!mctx && n_past > 0 && n_past < slot.prompt.n_tokens()) {
                                ...
                                if (pos_min > pos_min_thold) {
                                    ...
                                    const auto it = std::find_if(slot.prompt.checkpoints.rbegin(),
                                                                 slot.prompt.checkpoints.rend(), [&](const auto & cur) {
                                    ...
                                    if (!do_reset) {
                                        ...
                                        if (n != checkpoint_size) {
                                            ...
                                            SLT_WRN(slot,
                                                    "restored context checkpoint (pos_min = %d, pos_max = %d, size = "
                                                    "%.3f MiB)\n",
                                                    it->pos_min, it->pos_max, (float) checkpoint_size / 1024 / 1024);
                                        }
                                    }
                    ...
                    if (slot.prompt.n_tokens() == slot.task->n_tokens()) {
                        ...
                        do_checkpoint = do_checkpoint && (slot.prompt.checkpoints.empty() ||
                                                          pos_max > slot.prompt.checkpoints.back().pos_max + 64);
                        ...
                        if (do_checkpoint) {
                            while (slot.prompt.checkpoints.size() >= (size_t) params_base.n_ctx_checkpoints) {
                                ...
                                SLT_WRN(
                                    slot,
                                    "erasing old context checkpoint (pos_min = %d, pos_max = %d, size = %.3f MiB)\n",
                            ...
                            SLT_WRN(
                                slot,
                                "created context checkpoint %d of %d (pos_min = %d, pos_max = %d, size = %.3f MiB)\n",
                                (int) slot.prompt.checkpoints.size(), params_base.n_ctx_checkpoints, cur.pos_min,
                                cur.pos_max, (float) cur.data.size() / 1024 / 1024);
                        }
                    }
                }
        ...
        for (int32_t i = 0; i < batch.n_tokens; i = i_next) {
            ...
            for (auto & slot : slots) {
                ...
                // Parallelize sampling, detokenization and token processing
                futures.emplace_back(std::async(std::launch::async, [&slot, tok_idx, accept_special_token, this]() mutable {
            ...
            std::vector<std::future<void>> dft_sample_futures;
            for (auto & slot : slots) {
                ...
                dft_sample_futures.emplace_back(std::async(std::launch::async, [&slot, n_draft, accept_special_token, this]() mutable {
                    const auto ids =
                    ...
                    std::vector<std::future<void>> dft_futures;
                    for (size_t i = 0; i < ids.size(); ++i) {
                        ...
                        dft_futures.emplace_back(std::async(std::launch::async, [&slot, result, accept_special_token, this]() mutable {
                            result.text_to_send =
                                common_token_to_piece(ctx, result.tok, accept_special_token(slot, result.tok));
                    ...
                    for (auto & fut : dft_futures) {
                        fut.get();
                    }

                    SLT_DBG(slot, "accepted %d/%d draft tokens, new n_tokens = %d\n", (int) ids.size() - 1,
                            (int) n_draft, slot.prompt.n_tokens());
                }));
            }

            for (auto & fut : dft_sample_futures) {
                fut.get();
            }
    ...
    bool server_context::load_model(const common_params & params) { return impl->load_model(params); }
    ...
    void server_context::terminate() { impl->queue_tasks.terminate(); }

    llama_context * server_context::get_llama_context() const { return impl->ctx; }

    server_response_reader server_context::get_response_reader() { return impl->get_response_reader(); }
    ...
    void server_routes::init_routes() {
        ...
        this->get_metrics = [this](const server_http_req & req) {
            ...
            if (!params.endpoint_metrics) {
                res->error(
                    format_error_response("This server does not support metrics endpoint. Start it with `--metrics`",
        ...
        this->post_infill = [this](const server_http_req & req) {
            ...
            if (!err.empty()) {
                res->error(format_error_response(
                    string_format("Infill is not supported by this model: %s", err.c_str()), ERROR_TYPE_NOT_SUPPORTED));
                return res;
            }
            ...
            data["prompt"] = format_prompt_infill(
                ctx_server.vocab, data.at("input_prefix"), data.at("input_suffix"), data.at("input_extra"),
                params.n_batch, params.n_predict, meta->slot_n_ctx, params.spm_infill,
            ...
            json models = {
                { "models",
                 { { { "name", meta->model_name },
                      ...
                      { "tags", { "" } },
                      { "capabilities",
                        meta->has_mtmd ? json({ "completion", "multimodal" }) : json({ "completion" }) },
                      { "parameters", "" },
                      { "details",
                        { { "parent_model", "" },
                          { "format", "gguf" },
        ...
        this->post_rerank = [this](const server_http_req & req) {
            ...
            if (documents.empty()) {
                res->error(format_error_response("\"documents\" must be a non-empty string array",
                                                 ERROR_TYPE_INVALID_REQUEST));
                return res;
            }
    ...
    std::unique_ptr<server_res_generator> server_routes::handle_slots_restore(const server_http_req & req,
                                                                              int                     id_slot) {
        auto        res          = create_response();
        const json  request_data = json::parse(req.body);
        std::string filename     = request_data.at("filename");
    ...
    std::unique_ptr<server_res_generator> server_routes::handle_embeddings_impl(const server_http_req & req,
        ...
        if (body.count("encoding_format") != 0) {
            ...
            if (format == "base64") {
                ...
                res->error(
                    format_error_response("The format to return the embeddings in. Can be either float or base64",
                                          ERROR_TYPE_INVALID_REQUEST));
                return res;
            }
        }
