...
class ChatSidebarProvider {
    constructor(_extensionUri) {
        ...
        logger_1.CentralLogger.getInstance().debug('system', 'ChatSidebarProvider: Initializing...');
    ...
    resolveWebviewView(webviewView, _context, _token) {
        ...
        this._watchdogTimer = setTimeout(() => {
            if (!this._isReady) {
                logger_1.CentralLogger.getInstance().info('system', 'Gravitas Chat: Handshake timeout. Sidebar may be unresponsive or blocked by CSP.');
            }
        }, 5000);
        // 🛡️ Lifecycle Cleanup
    ...
    postOrBuffer(message) {
        if (this._isReady && this._view) {
            this._view.webview.postMessage(message);
        }
        else {
            logger_1.CentralLogger.getInstance().debug('system', `ChatSidebarProvider: Buffering message of type ${message.type}. Ready: ${this._isReady}, View: ${!!this._view}`);
    ...
    showTask(taskId) {
        let task;
        if (taskId === 'last') {
            task = taskManager_1.TaskManager.getInstance().getLastTask();
        }
        else {
            task = taskManager_1.TaskManager.getInstance().getTask(taskId);
        }
            ...
            switch (message.type) {
                case 'ready': {
                    ...
                    if (this._messageBuffer.length > 0) {
                        logger_1.CentralLogger.getInstance().info('system', `Gravitas Chat: Flushing ${this._messageBuffer.length} buffered events.`);
                        this._messageBuffer.forEach(msg => webview.postMessage(msg));
                        this._messageBuffer = [];
                    }
                    // 2. Initial State Sync
                    const lastTask = taskManager_1.TaskManager.getInstance().getLastTask();
                    if (lastTask) {
                ...
                case 'abortTask': {
                    if (message.taskId) {
                        taskManager_1.TaskManager.getInstance().abortTask(message.taskId);
                    }
                    break;
                }
                case 'error': {
                    logger_1.CentralLogger.getInstance().error('system', `Gravitas Chat Frontend Error: ${message.message}\n${message.stack}`);
                    break;
                }
            }
        });
...
ChatSidebarProvider.viewType = 'gravitas.chat';
function getNonce() {
    let text = '';
